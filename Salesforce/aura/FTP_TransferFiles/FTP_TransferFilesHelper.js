({
	transferFile : function(component, event) {
		this.startWaiting(component);
        var action = component.get("c.transferFilesFronOneFtpServertoOtherFtpServer");
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                let result = response.getReturnValue();
                component.set("v.oFTPResponseWrapper", result);
                //this.showToastMessage(component, event, 'Success', "File Transfered Successfully!", 'success');
            }else if (state === "ERROR") {
